    ...
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        textAreaInput(
          ...
          placeholder = "You can paste plain text OR include [SPEECH] and [REPORT] sections here...",
          rows = 12
        )
      ),
      
      helpText(
        "You can either upload a text file OR directly paste/type text.
         For fraud analysis, include [SPEECH] and [REPORT] sections."
      )
    ),
...
server <- function(input, output) {
  ...
  text_vector <- reactive({
    ...
    if (input$input_mode == "text") {
      req(input$manual_text)
      txt <- unlist(strsplit(input$manual_text, "\n"))
    } else {
      req(input$file)
      txt <- readLines(input$file$datapath, encoding = "UTF-8")
    }
